...
class CCharsRecognise {
 public:
  ...
  inline std::string getPlateColor(cv::Mat input) const {
    ...
    if (BLUE == result) color = "BlueLicenselate";
    if (YELLOW == result) color = "YellowLicenselate";
    if (WHITE == result) color = "WhiteLicenselate";
#ifdef OS_WINDOWS
  ...
  inline std::string getPlateColor(Color in) const {
    std::string color = "UNKNOWN";
    if (BLUE == in) color = "BlueLicenselate";
    if (YELLOW == in) color = "YellowLicenselate";
    if (WHITE == in) color = "WhiteLicenselate";
#ifdef OS_WINDOWS
    color = utils::utf8_to_gbk(color.c_str());
#endif
    return color;
